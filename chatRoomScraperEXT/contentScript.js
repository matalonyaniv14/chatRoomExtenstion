...
const findIndexOf = (message, messageList) => {
  ...
  if (author && content) {
    return messageList.findIndex((m) => {
      // return m.author === message?.author && m.content === message?.content;
      return (
        m.author.match(new RegExp(author)) &&
        m.content.match(new RegExp(content))
      );
    });
  } else {
    return -1;
...
const findMostRecent = async () => {
  ...
  if (mostRecentServerMessage) {
    ...
    let recentMessageIndex = findIndexOf(
      mostRecentServerMessage,
      clientMessages
    );
